...
def precisione():
    """Su T4 (cc 7.5) bf16 non esiste in hardware. torch.cuda.is_bf16_supported()
    ...
    return ("bf16", torch.bfloat16) if cc[0] >= 8 else ("fp16", torch.float16)
    ...
    def on_log(self, args, state, control, logs=None, **kw):
        ...
        if v is not None and not math.isfinite(v):
            print(f"\n! loss non finita ({v}) allo step {state.global_step}: "
                  ...
                  "bf16 (Gemma).\n"
                  "  Non si risolve abbassando il learning rate: o si usa una "
                  "GPU Ampere\n  (compute capability >= 8, dove parte il bf16) "
                  "oppure si sceglie un altro modello.", flush=True)
...
def main():
    ...
    if a.peso_prefisso > 0:
        ...
        print("  Ordine di grandezza atteso: la train loss e' PIU' BASSA della "
              "eval loss di circa il\n  fattore (peso medio dei token). Se "
              "invece la vedi molte volte PIU' ALTA (es. 40 contro 3)\n"
              "  non e' l'asimmetria: e' la normalizzazione della loss "
              "sull'accumulo, vedi TrainerPesato.")
        print("  Ordine di grandezza atteso: la train loss e' PIU' BASSA della "
    ...
    attn = "eager" if "gemma" in repo.lower() else "sdpa"
