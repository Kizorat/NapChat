...
from Minerva7B.T1_Traduzione.training.common import load_split, render_prompt, resolve_model
from Minerva7B.T1_Traduzione.training.contesto_metrica import estrai_blocco_contesto, sostituisci_contesto
from Minerva7B.T1_Traduzione.training.evaluate_task import MAX_NEW, id_fine_turno, norm
...
def ablazione(model, tok, torch, coppie, max_new, eot, batch=8, limite=64,
    ...
    """
    import sacrebleu
    from Minerva7B.T1_Traduzione.training.evaluate_task import pulisci_generato

    sub = coppie[:limite]
    prev = tok.padding_side
    tok.padding_side = "left"
    gen_kw = dict(max_new_tokens=max_new, min_new_tokens=1, do_sample=False,
                  num_beams=1, eos_token_id=eot, pad_token_id=tok.pad_token_id)

    def genera(testi):
        out = []
        for i in range(0, len(testi), batch):
            b = testi[i:i + batch]
            enc = tok([render_prompt(tok, x) for x in b], return_tensors="pt",
                      padding=True, add_special_tokens=False).to(model.device)
            with torch.no_grad():
                g = model.generate(**enc, **gen_kw)
            w = enc["input_ids"].shape[1]
            out += [pulisci_generato(tok.decode(s[w:], skip_special_tokens=True))
                    for s in g]
            print(f"    {min(i + batch, len(testi))}/{len(testi)}", end="\r")
        return out
...
def main():
    ...
    from Minerva7B.T1_Traduzione.training.common import load_backbone, load_hf_token
