...
def load_backbone(repo_id, dtype, **kw):
    ...
    try:
        return from_pretrained_compat(AutoModelForCausalLM, repo_id, dtype, **kw), "CausalLM"
        ...
        return from_pretrained_compat(AutoModelForImageTextToText, repo_id, dtype, **kw), \
            "ImageTextToText"
        ...
        def on_evaluate(self, args, state, control, model=None, metrics=None, **kwargs):
            ...
            try:
                for i in range(0, len(sub), batch):
                    b = sub[i:i + batch]
                    testi = [render_prompt(tokenizer, r["prompt"]) for r in b]
                    enc = tokenizer(testi, return_tensors="pt", padding=True,
                                    add_special_tokens=False).to(model.device)
                    with torch.no_grad():
                        g = model.generate(**enc, max_new_tokens=task.gen_max_new_tokens,
                                           do_sample=False, num_beams=1,
                                           pad_token_id=tokenizer.pad_token_id)
                    for j in range(len(b)):
                        txt = tokenizer.decode(g[j][enc["input_ids"].shape[1]:],
    ...
    class SampleGenerationCallback(TrainerCallback):
        def on_evaluate(self, args, state, control, model=None, **kwargs):
            ...
            for prompt, target in samples:
                enc = tokenizer(render_prompt(tokenizer, prompt), return_tensors="pt",
                                add_special_tokens=False).to(model.device)
                with torch.no_grad():
                    out = model.generate(**enc, max_new_tokens=task.gen_max_new_tokens,
                                         do_sample=False,
                                         repetition_penalty=task.repetition_penalty,
                                         no_repeat_ngram_size=task.no_repeat_ngram_size,
                                         pad_token_id=tokenizer.pad_token_id)
...
def run(task: TaskConfig):
    ...
    print(f"GPU: {gpu_name} (cc {_cc[0]}.{_cc[1]}), {vram_gb:.1f} GB VRAM | bf16 nativo: {bf16_ok} -> precisione: {'bf16' if bf16_ok else 'fp16'}")
    if not bf16_ok:
        print("  GPU pre-Ampere: si usa fp16 (tensor core nativi). Gemma-2 in fp16 può divergere per il "
              "logit soft-capping; se compare loss=nan, nell'ordine: --lr 5e-5, "
              "--max-seq-len più basso, poi --model gemma-small documentando la sostituzione.")
    ...
    lex = None
    if args.lessico:
        from Minerva7B.T1_Traduzione.training.pesi_lessicali import (ChatDatasetPesato, carica_lessico,
                                    make_collate_pesato)
    ...
    if args.gen_metric_n > 0:
        callbacks.append(build_generation_metric_callback(
            TrainerCallback, torch, tokenizer, dev_rows, task, n=args.gen_metric_n))
    ...
    if args.ctx_metric_n > 0:
        from Minerva7B.T1_Traduzione.training.contesto_metrica import build_context_callback
    ...
    if lex:
        from Minerva7B.T1_Traduzione.training.pesi_lessicali import TrainerPesato
